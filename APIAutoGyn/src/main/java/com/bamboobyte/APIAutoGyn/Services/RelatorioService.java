...
public class RelatorioService {
    ...
    private static final NumberFormat FORMATADOR_REAIS = NumberFormat.getCurrencyInstance(LOCALE_BR);
    ...
    public byte[] gerarRelatorio(OS os) {
        try {
            ...
            doc.add(new Paragraph("Status: " + (os.getEtapa() != null ? os.getEtapa().getDescricao() : "Indefinido"),
                    textoFont));
            ...
            if (os.getItensServico() != null && !os.getItensServico().isEmpty()) {
                for (var item : os.getItensServico()) {
                    String inicio = item.getDataInicio() != null ? FORMATADOR_DATA.format(item.getDataInicio())
                            : "Início não informado";
                    String fim = item.getDataFim() != null ? FORMATADOR_DATA.format(item.getDataFim()) : "Em andamento";
                    String responsavel = item.getColaborador() != null ? item.getColaborador().getNome()
                            : "Sem responsável";
            ...
            if (os.getItensPeca() != null && !os.getItensPeca().isEmpty()) {
                for (var item : os.getItensPeca()) {
                    doc.add(new Paragraph(String.format("- %s | Qtd: %d | Valor unitário: %s",
                            item.getPeca().getDescricao(), item.getQuantidade(),
                            formatarReais(item.getValorUnitario())), textoFont));
                }
            } else {
            ...
            doc.add(new Paragraph("Valor total: " + formatarReais(os.getValorTotal()), textoFont));
            doc.add(new Paragraph("Valor pago: " + formatarReais(os.getValorPago()), textoFont));
